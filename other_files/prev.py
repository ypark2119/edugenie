...
#                     # === Ensure dtstart and dtend are datetime (not date)
...
#                     is_all_day = dtstart.time() == datetime.min.time()
...
#                 parsed_events = deduplicate_events(parsed_events)
#                 assignments = [e for e in parsed_events if e.get("allDay")]
#                 classes = [e for e in parsed_events if not e.get("allDay")]

#                 # === Save to correct calendar file
#                 try:
#                     if calendar_type == "Assignments":
#                         blob = gcs_client().bucket(BUCKET_NAME).blob("calendar_assignments.json")
#                         existing = json.loads(blob.download_as_text()) if blob.exists() else []
#                         combined = deduplicate_events(existing + assignments)
#                         blob.upload_from_string(json.dumps(combined, indent=2))

#                     elif calendar_type == "Classes":
#                         blob = gcs_client().bucket(BUCKET_NAME).blob("calendar_classes.json")
#                         existing = json.loads(blob.download_as_text()) if blob.exists() else []
#                         combined = deduplicate_events(existing + classes)
#                         blob.upload_from_string(json.dumps(combined, indent=2))

#                     st.success(f"✅ {calendar_type} calendar events saved.")
#                 except Exception as e:
#                     st.error("❌ Failed to update calendar events.")
#                     st.exception(e)
...
#         e for e in events
#         if e.get("allDay", False) == is_all_day_view and
#         today <= parse(e.get("start")).date() <= seven_days_later
#     ]
#     filtered = sorted(filtered, key=lambda x: x["start"])
