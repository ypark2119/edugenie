...
st.set_page_config(page_title="Smart Study Schedule Generator", layout="wide")
st.title("📚 Smart Study Schedule Generator")

# === Step 1: Input iCal URL ===
ical_url = st.text_input("📅 Paste your public iCal URL (e.g. Google Calendar with assignments)", placeholder="https://...")
...
if ical_url:
    try:
        ...
        ical_text = r.text.replace('\r\n', '\n').strip()
        cal = Calendar(ical_text)
        for event in cal.events:
            # Default duration for each task can be adjusted later
            due = event.begin.datetime if hasattr(event.begin, 'datetime') else arrow.get(event.begin).datetime
            assignments.append({
                "title": event.name or "Untitled Task",
                "due": due,
                "duration_hrs": 2  # Default duration per task
            })
        if not assignments:
            st.warning("No upcoming events found.")
        else:
            st.success(f"✅ Loaded {len(assignments)} assignment(s) from calendar.")
    except Exception as e:
        st.error("❌ Failed to parse iCal. Please check the URL or formatting.")
        st.exception(e)

# === Step 2: Input availability ===
with st.form("availability_form"):
    st.subheader("🕒 Your Weekly Availability")
    st.caption("How many hours can you study on each day?")
    days = ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"]
    availability_input = {day: st.slider(day, 0, 8, 2) for day in days}
    submitted = st.form_submit_button("📅 Generate Study Plan")

# === Step 3: Greedy scheduling algorithm ===
def generate_schedule(assignments, availability):
    schedule = []
    today = datetime.today()

    for task in sorted(assignments, key=lambda x: x["due"]):
        hours_needed = task["duration_hrs"]
        current_day = task["due"].date()

        while hours_needed > 0 and current_day >= today.date():
            day_name = current_day.strftime("%A")
            available_hours = availability.get(day_name, 0)

            if available_hours > 0:
                hrs_to_schedule = min(hours_needed, available_hours)
                schedule.append({
                    "Task": task["title"],
                    "Start": datetime.combine(current_day, datetime.min.time()),
                    "End": datetime.combine(current_day, datetime.min.time()) + timedelta(hours=hrs_to_schedule)
                })
                hours_needed -= hrs_to_schedule
                availability[day_name] -= hrs_to_schedule
...
if submitted and assignments:
    availability_copy = availability_input.copy()  # Avoid mutating original
    df_schedule = generate_schedule(assignments, availability_copy)

    if df_schedule.empty:
        st.warning("⚠️ Not enough time in your schedule to fit all tasks.")
    else:
        st.subheader("📈 Your Personalized Study Plan")
        fig = px.timeline(df_schedule, x_start="Start", x_end="End", y="Task", title="Study Plan")
        fig.update_yaxes(autorange="reversed")
        fig.update_layout(height=500, margin=dict(l=20, r=20, t=40, b=20))
        st.plotly_chart(fig, use_container_width=True)

        with st.expander("📋 View Scheduled Tasks as Table"):
...
#     reparse_calendar = ical_url and (st.session_state["prev_ical_url"] != ical_url or reload_requested)
#     if reparse_calendar:
...
#                 # parsed_events = []
#                 # now = arrow.utcnow()
#                 # future_limit = now.shift(days=+120)

#                 # for e in cal.timeline.included(now, future_limit):
#                 #     start = get_event_start(e)
#                 #     end = get_event_end(e)

#                 #     is_all_day = not isinstance(start, datetime) or start.time() == datetime.min.time()
...
#                 future_limit = now + timedelta(days=120)
...
#                     summary = str(component.summary.value) if hasattr(component, 'summary') else "Untitled Event"
#                     dtstart = component.dtstart.value
#                     dtend = component.dtend.value if hasattr(component, 'dtend') else dtstart
#                     is_all_day = not isinstance(dtstart, datetime) or dtstart.time() == datetime.min.time()

#                     rset = rruleset()
#                     if hasattr(component, "rrule"):
#                         rset.rrule(rrulestr(component.rrule.value, dtstart=dtstart))
#                     else:
#                         rset.rdate(dtstart)

#                     if hasattr(component, "exdate"):
#                         for ex in component.exdate.value:
#                             rset.exdate(ex)

#                     for occurrence in rset.between(now, future_limit, inc=True):
#                         start_str = occurrence.date().isoformat() if is_all_day else occurrence.isoformat()
#                         end_dt = dtend if isinstance(dtend, datetime) else datetime.combine(dtend, datetime.max.time())
#                         duration = end_dt - dtstart
#                         end_occurrence = occurrence + duration
#                         end_str = end_occurrence.date().isoformat() if is_all_day else end_occurrence.isoformat()
...
#     filtered = [
#         e for e in events
#         if e.get("allDay", False) == is_all_day_view and
#         today <= datetime.fromisoformat(e.get("start")).date() <= seven_days_later
#     ]
...
#             start_str = datetime.combine(c_date, c_start).replace(tzinfo=None).isoformat()
#             end_str = datetime.combine(c_date, c_end).replace(tzinfo=None).isoformat()
